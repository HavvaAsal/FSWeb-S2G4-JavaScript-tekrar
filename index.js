...
sayilar.forEach((sayi) => {
  sayacObj[sayi] = (sayacObj[sayi] || 0) + 1;
});

for (let sayi in sayacObj) {
  if (sayacObj[sayi] > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${sayacObj[sayi]} kere tekrar edilmiştir.`);
  }
}

console.log("Tekrar eden tüm sayılar:", tekraredensayilar);
